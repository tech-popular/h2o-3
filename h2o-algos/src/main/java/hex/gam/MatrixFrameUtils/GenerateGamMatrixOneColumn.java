...
public class GenerateGamMatrixOneColumn extends MRTask<GenerateGamMatrixOneColumn> {
  ...
  public GenerateGamMatrixOneColumn(int splineType, int numKnots, double[] knots, Frame gamx, boolean standardize) {
    ...
    // change the spline instantiation when we support more than one spline type
     CubicRegressionSplines crSplines = new CubicRegressionSplines(numKnots, knots, gamx);
  ...
  public void map(Chunk[] chk, NewChunk[] newGamCols) {
    ...
    CubicRegressionSplines crSplines = new CubicRegressionSplines(_numKnots, _knots, _gamX); // not iced, must have own
